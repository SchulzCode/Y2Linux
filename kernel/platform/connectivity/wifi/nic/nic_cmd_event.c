...
/*----------------------------------------------------------------------------*/
VOID
nicCmdEventQueryMemDump(IN P_ADAPTER_T adapter, IN P_CMD_INFO_T command,
                       IN PUINT_8 event)
{
    /* Memory dumps are outside the production cfg80211 interface. */
    if (command->fgIsOid)
        kalOidComplete(adapter->prGlueInfo, command->fgSetQuery, 0,
                       WLAN_STATUS_NOT_SUPPORTED);
}
