    ...
    )
{
    ...
    do {
        ...
        /* <1> Open firmware */
        i4Ret = y2_conn_request_firmware(prHifInfo->Dev,
        ...
        if (i4Ret) {
            ...
            *ppvMapFileBuf = (void *)prGlueInfo->prFw->data; /* legacy engine only reads this buffer */
            return (void *)prGlueInfo->prFw->data;
        }
        ...
        if(prGlueInfo->fgIsRegistered == TRUE) {
            ...
            if (fgWasConnected) {
                struct cfg80211_roam_info roam = {
                    .req_ie=prGlueInfo->aucReqIe,.req_ie_len=prGlueInfo->u4ReqIeLength,
                    .resp_ie=prGlueInfo->aucRspIe,.resp_ie_len=prGlueInfo->u4RspIeLength,
                };
                roam.links[0].channel=prChannel; roam.links[0].bssid=arBssid;
                cfg80211_roamed(prGlueInfo->prDevHandler,&roam,GFP_KERNEL);
            }
            if (bss)
    ...
    case WLAN_STATUS_SCAN_COMPLETE:
        ...
        case WLAN_STATUS_CONNECT_INDICATION:
            ...
            cfg80211_connect_result(prGlueInfo->prDevHandler,
                    ...
                    prGlueInfo->prAdapter->rWifiVar.rAisFsmInfo.prTargetBssDesc->aucBSSID : NULL,
                    prGlueInfo->aucReqIe,
                    prGlueInfo->u4ReqIeLength,
                    prGlueInfo->aucRspIe,
                    prGlueInfo->u4RspIeLength,
    ...
    )
{
    ...
    DBGLOG(TX, EVENT, ("----- pending frame %d -----\n", (INT_32)prGlueInfo->i4TxPendingFrameNum));
...
WLAN_STATUS
kalIoctl(IN P_GLUE_INFO_T glue, IN PFN_OID_HANDLER_FUNC handler,
    IN PVOID buffer, IN UINT_32 length, IN BOOL read, IN BOOL wait,
    IN BOOL command, IN BOOL p2p, OUT PUINT_32 result_length)
{
    P_GL_IO_REQ_T req = &glue->OidEntry;
    WLAN_STATUS result = WLAN_STATUS_ADAPTER_NOT_READY;
    void *owned;
    size_t extra = 0;

    /* This station-only port has one pointer-bearing request: scan IEs.
     * Copy those as well. Nothing queued may retain a caller's stack or
     * cfg80211 request after the bounded wait expires. */
    if (length > 65536 || (length && !buffer) || !result_length)
        return WLAN_STATUS_INVALID_DATA;
    if (handler == wlanoidSetBssidListScanExt) {
        P_PARAM_SCAN_REQUEST_EXT_T scan = buffer;
        if (length != sizeof(*scan) || scan->u4IELength > MAX_IE_LENGTH ||
            (scan->u4IELength && !scan->pucIE)) return WLAN_STATUS_INVALID_DATA;
        extra = scan->u4IELength;
    }
    if (kalIsResetting()) return result;
    if (down_interruptible(&g_halt_sem)) return WLAN_STATUS_FAILURE;
    if (g_u4HaltFlag) goto halt_unlock;
    if (down_interruptible(&glue->ioctl_sem)) goto halt_unlock;
    if (req->abandoned || kalIsResetting()) goto unlock;
    owned = kzalloc(max_t(size_t, 1, length + extra), GFP_KERNEL);
    if (!owned) { result = WLAN_STATUS_RESOURCES; goto unlock; }
    if (length) memcpy(owned, buffer, length);
    if (handler == wlanoidSetBssidListScanExt) {
        P_PARAM_SCAN_REQUEST_EXT_T scan = owned;
        if (extra) memcpy(owned + length, scan->pucIE, extra);
        scan->pucIE = extra ? owned + length : NULL;
    }
    req->prAdapter = glue->prAdapter;
    req->pfnOidHandler = handler;
    req->pvInfoBuf = owned;
    req->u4InfoBufLen = length;
    req->owned_result_length = 0;
    req->pu4QryInfoLen = &req->owned_result_length;
    req->fgRead = read;
    req->fgWaitResp = wait;
    req->rStatus = WLAN_STATUS_FAILURE;
    req->u4Flag = command;
    glue->rPendStatus = WLAN_STATUS_FAILURE;
    reinit_completion(&glue->rPendComp);
    set_bit(GLUE_FLAG_OID_BIT, &glue->u4Flag);
    wake_up_interruptible(&glue->waitq);
    if (!wait_for_completion_timeout(&glue->rPendComp, 10 * HZ)) {
        /* Keep the owned buffer until remove has drained every producer.
         * Return promptly so recovery can acquire cfg80211/RTNL locks. */
        req->abandoned = TRUE;
        y2_wifi_error(glue);
        goto unlock;
    }
    result = req->rStatus == WLAN_STATUS_PENDING ? glue->rPendStatus : req->rStatus;
    *result_length = req->owned_result_length;
    if (read && length) memcpy(buffer, owned, length);
    kfree_sensitive(owned);
    req->pvInfoBuf = NULL;
unlock:
    up(&glue->ioctl_sem);
halt_unlock:
...
{
    P_GLUE_INFO_T prGlueInfo = timer_container_of(prGlueInfo, timer, tickfn);
