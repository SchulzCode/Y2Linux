    ...
    )
{
    ...
    rStatus = kalIoctl(prGlueInfo, wlanoidQueryStatisticsPL, &rStatistics,
                      sizeof(rStatistics), TRUE, TRUE, TRUE, FALSE, &u4BufLen);
    if (rStatus == WLAN_STATUS_SUCCESS) {
        sinfo->filled |= BIT_ULL(NL80211_STA_INFO_TX_PACKETS) |
                         BIT_ULL(NL80211_STA_INFO_TX_FAILED) |
                         BIT_ULL(NL80211_STA_INFO_TX_RETRIES);
        sinfo->tx_packets = rStatistics.rTransmittedFragmentCount.QuadPart;
        sinfo->tx_failed = rStatistics.rFailedCount.QuadPart;
        sinfo->tx_retries = rStatistics.rRetryCount.QuadPart;
    }
    return 0;
}
