    ...
    )
{
    ...
    if (prAdapter->fgIsEmbbededMacAddrValid &&
        is_valid_ether_addr(prAdapter->rWifiVar.aucPermanentAddress))
        return WLAN_STATUS_SUCCESS;
    if (!kalRetrieveNetworkAddress(prAdapter->prGlueInfo,&rMacAddr) ||
        !is_valid_ether_addr(rMacAddr)) return WLAN_STATUS_INVALID_DATA;
    ...
    )
{
    ...
    for (i=0; i<14;i++)
        prTxpwr->acTxPwrLimit2G[i] = 40; /* cfg80211 world ceiling; later notifier can only apply legal channel ceilings */
