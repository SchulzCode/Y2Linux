...
int main(void) {
    ...
    assert(irq_count(out,sizeof(out)," 1: 77 GIC x other\n 2: 1024 GIC 112 Level timer@10008000\n")==0);
    assert(!strcmp(out,"1024"));
    assert(irq_count(out,sizeof(out),"IPI0: 1 unrelated\n")==-61);
    assert(irq_count(out,2,"1: 999 GIC 112 Level /timer@10008000\n")==-75);
    assert(irq_count(out,sizeof(out),"  1: 77 GIC 112 Level /timer@10008000\n")==0);
    assert(!strcmp(out,"77"));
    assert(irq_count(out,sizeof(out),"1: 42 GIC timer@100080000\n")==-61);
    assert(irq_count(out,sizeof(out),"1: 42 GIC othertimer@10008000\n")==-61);
    assert(irq_count(out,sizeof(out),"1: 42 GIC timer@10009000\n")==-61);
    assert(irq_count(out,sizeof(out),"Err: 42 timer@10008000\n")==-61);
    assert(irq_count(out,sizeof(out),"1: nope GIC timer@10008000\n")==-61);
    assert(irq_count(out,sizeof(out),"1: 123x GIC timer@10008000\n")==-61);
    assert(irq_count(out,sizeof(out),"1: 42 GIC mtk-clkevt\n")==-61);
