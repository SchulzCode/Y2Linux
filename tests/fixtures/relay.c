...
static void run(void) {
    ...
    CHECK(contains("Y2LOG1 Y2LINUX-DEV-01\nPID1 startup\n"));
    ...
    write_error=0;relay.result=0;output_n=0;relay.resume=1;command("");
    unsigned prior=seeked;service();
    CHECK(relay.active && !relay.resume && seeked==prior+1 && contains("Y2LOG1 Y2LINUX-DEV-01\n"));
    service();CHECK(seeked==prior+1);
    relay_close();
