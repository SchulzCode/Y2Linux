...
For [M2-USBACM-01](m2-usbacm-01-result.md), Linux host, standard Python 3 only.
Run from the repository **before powering on the test image**:

```sh
python3 tools/observation/usb_log_capture.py --output out/m2-usbacm-01/capture-01
```
...
This host wait does not extend the Y2's60s-from-power-on observation limit.
...
trial. If the prompt never appears, return a readable screen photo and do not
try connected-start recovery. The old connected-start PHY6a=BE guard remains.
...
Capture may end on the candidate's50s active-window disconnect; preserve the
...
- `raw.bin`: exact returned bytes, including native `/dev/kmsg` records and
  PID1 startup/heartbeat status. The first line should identify M2-USBACM-01.
