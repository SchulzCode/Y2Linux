...
## Later owner authorization: M1 offline wave

The owner changed the M1 execution model to direct implementation by the lead, without delegation. [M1 offline checkpoints](M1-first-boot.md) are now implemented and validated against D08. This supersedes earlier implementation-role and no-artifact restrictions for offline work only. M0 hardware/recovery gates remain unsatisfied; actual artifact bounds are now resolved, while installed-loader/security, inherited-state, console, backups and power remain required before device launch.
