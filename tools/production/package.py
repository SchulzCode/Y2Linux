...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--build',type=Path,default=PROJECT/'out/y2linux-production-build-v1');p.add_argument('--output',type=Path,default=PROJECT/'out/y2linux-production-v1');p.add_argument('--public-key',type=Path,required=True);a=p.parse_args()
    ...
    for name,target in TARGETS.items():
        ...
        if name!='USRDATA':entry['requires']['kernel_module_release']=versions['kernel_version']
        if name!='BOOTIMG':
            spft=raw.with_suffix('.spft.img');sparse_encode(raw,spft);expanded=sparse_identity(spft,strict=True)
            require(expanded=={'expanded_bytes':raw.stat().st_size,'expanded_sha256':entry['raw']['sha256']},'sparse expanded identity')
            entry.update(filesystem={'type':'ext4','label':target['label'],'uuid':target['uuid']},spft={'file':spft.name,'format':'android-sparse-v1-raw-fill','size_bytes':spft.stat().st_size,'sha256':digest(spft),**expanded})
    ...
    manifest={'schema':'org.schulzcode.y2linux.release/v1',**versions,'status':'offline-validated candidate; manual physical acceptance pending','hardware_compatibility':{'vendor':'Innioasis','board':'Y2 eastaeon82_wet_kk','soc':'MT6582','emmc_user_capacity_bytes':7784103936,'stock_scatter_sha256':STOCK_SCATTER_SHA256,'other_revisions':'unsupported until their layout and hardware are verified'},'payloads':payloads,'normal_update_allowlist':['BOOTIMG','ANDROID'],'data_policy':'USRDATA persists; data template only for explicit first initialization/factory reset','runtime_kernel_write_allowlist':['ANDROID','USRDATA'],'application':{'name':'Y2PlayerNative','version':None,'implemented':False,'builtin_path':'/usr/bin/y2player','future_payload_root':'/data/apps/y2player','state_root':'/data/y2player','partition':None},'minimum_compatible_components':{'layout_version':1,'data_schema_version':1,'rootfs_abi':'armv7-eabihf-glibc','kernel_contract':'y2-storage-v1','rootfs_contract':'y2-storage-v1'},'preserved_paths':['/data','external microSD'],'restoration_sources':restoration,'debug_access':{'scope':'owner-personalized first-install data image; NOT a universal production credential','authorized_public_key_sha256':digest(a.public_key),'private_key_packaged':False,'host_keys':'generated on device and persisted on Y2DATA'},'future_security':{'signature':None,'signing_key_id':None,'update_channel':None,'rollback_index':None},'profiles':{n:{'sha256':digest(out/n),'selected_partitions':parts} for n,parts in [('MT6582_Android_scatter.txt',['BOOTIMG','ANDROID','USRDATA']),('MT6582_preserve_data_scatter.txt',['BOOTIMG','ANDROID'])]}}
