...
def check(data, initrd_size):
    ...
    require(tuple(reserved)==RESERVED, 'D08 reservations changed')
    require(nodes['/memory@80000000']['reg']==cells(0x80000000,0x1800000,0x84000000,0x80000), 'D08 RAM changed')
    chosen=nodes['/chosen']
    ...
    require(args=='rdinit=/init earlycon console=ttyS0,921600n8 console=tty0 loglevel=8 ignore_loglevel panic=0 log_buf_len=256K initcall_debug', 'baseline command line')
    ...
    for path in ('/mmc@11230000','/mmc@11240000'):
        v=nodes[path]
        require(v['compatible']==strings('innioasis,y2-msdc-readonly') and v['bus-width']==cells(1) and v['max-frequency']==cells(400000),'storage discovery contract')
    ...
    return {'node_count':len(nodes),'bootargs':args,'memory':RAM,'storage':'identity only; no block driver','evidence':'offline dependencies only'}
