...
def check(kernel):
    ...
    names=('y2_usb_worker','y2_musb_init','y2_musb_interrupt','y2_session_write',
           'musb_start','musb_stop','musb_g_disconnect','musb_interrupt','musb_g_ep0_irq','acm_bind','gserial_connect')
    names+=('cdc_bind','ecm_bind') if 'cdc_bind' in kernel.syms else ('gs_bind',)
